...
**Update 2026-08-17: Phase 1 (BUG-01), the calculation/data-integrity portion of Phase 2, and the Phase 3 currency design have all been executed and verified live** — see `03-bug-register.md` for per-bug status, `09-bug01-regression-report.md` for the Phase 1 backfill detail, and `02-calculation-audit.md` §13 for the Phase 3 currency design detail. Specifically done: BUG-01 (Phase 1A code fix + Phase 1B historical backfill), BUG-02, BUG-03, BUG-04, BUG-05, BUG-12, BUG-13, BUG-14, BUG-20, BUG-22 (Phase 2), and BUG-06/BUG-19 (Phase 3 — the EUR customer-pricing vs. USD internal-reporting currency design: an `exchange_rates` cache table, new explicit `usd_*` columns on `sales`/`sale_items`, an ECB-derived no-API-key rate source with a safe manual fallback, one snapshotted rate applied consistently across a whole sale, and a verified historical backfill of all 34 existing sales). Deliberately **not** done: BUG-09/Phase 3's CSS cleanup and visual redesign, and BUG-07/BUG-08/BUG-10/BUG-11/BUG-21 (dead-code/hardening cleanup that isn't a calculation or data-integrity defect). The plan below is preserved as originally written for the remaining phases.
