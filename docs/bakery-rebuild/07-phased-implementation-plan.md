...
**Update 2026-08-17: Phases 1 through 5 (BUG-01, Phase 2, Phase 3 currency, BUG-23, Phase 4 admin reorganization/CSS refactor, and Phase 5 public-site polish/CSS cleanup) have all been executed and verified live** — see `03-bug-register.md` for per-bug status, `09-bug01-regression-report.md` for the Phase 1 backfill detail, `02-calculation-audit.md` §13 for the Phase 3 currency design detail, and `04-admin-ux-audit.md`/`05-css-audit.md` for the Phase 4/5 detail. Specifically done: BUG-01 (Phase 1A + 1B), BUG-02/03/04/05/12/13/14/20/22 (Phase 2), BUG-06/19 (Phase 3 currency design) and BUG-23 (Production currency fix), BUG-07/08/09/11 (Phase 4 — admin reorganization: one shared, grouped, mobile-responsive navigation module replacing 13 hand-copied nav blocks; a CSS refactor consolidating 108 duplicate selector groups, fixing a real unclosed-`@media` bug that had silently disabled ~1,100 lines of Dashboard CSS at desktop widths; the stranded production CSS moved into a real `css/production.css`; `js/admin.js` deleted), and BUG-10 (Phase 5 — public-site polish: `css/style.css` consolidated (7 duplicate groups, confirmed dead selectors, three genuine bugs including an invalid duplicated-property-name declaration that had silently disabled `body`'s background gradient); a global focus-visible ring, reduced-motion support, and skip-links added to all 4 public pages; the one genuinely-duplicated piece of the Dashboard/Reviews pending-reviews logic extracted into a shared, tested module). Deliberately **not** done: BUG-21 (minor Supabase hardening, unrelated), and the broader per-page component/JS-behavior rework §2-§4 of `04-admin-ux-audit.md` describe (modal/accordion unification, semantic tables, inline form validation) — those remain open for a future pass. The plan below is preserved as originally written for the remaining phases.
