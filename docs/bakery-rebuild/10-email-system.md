...
**Incident, same day:** shortly after enabling `order_emails_enabled`, real checkout started failing with `new row violates row-level security policy for table "email_outbox"`. Root cause: `enqueue_order_received_email()`/`enqueue_order_status_email()` inserted into `email_outbox` without `SECURITY DEFINER`, so during a real anon checkout the insert ran as `anon` (which correctly has zero grants on `email_outbox`) and failed -- aborting the whole `order_items` insert inside the same transaction, since triggers run in the triggering statement's own transaction. The console's repeated `builder_details` 403s were this exact same failing statement, not a separate bug -- `builder_details` is just a field in the `order_items` payload. Fixed in `20260818143142_...`/`20260818143411_...`: both trigger functions are now `SECURITY DEFINER` with a fixed `search_path` and an exception handler that swallows (and logs a `WARNING` for) any future outbox-insert failure rather than ever again blocking a customer's order; grants were tightened to nothing for `anon`/`authenticated`/`PUBLIC` (verified live that PostgREST 404s trying to call either directly -- trigger-return-type functions aren't exposed as RPCs regardless of grants, so this was hygiene, not a real hole). `anon` still has zero table-level grant on `email_outbox`, unchanged. Re-verified end-to-end with a real anon-role checkout (order + items, including a `builder_details` item) after both migrations: 1 order, correct item count, exactly 1 `email_outbox` row, sent and webhook-confirmed delivered.
