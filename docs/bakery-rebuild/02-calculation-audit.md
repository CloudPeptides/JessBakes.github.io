...
## 12. Remaining uncertainty after database verification (resolved in Phase 3 — see §13)

- ~~The exchange-rate source is not yet decided.~~ **Resolved**: `api.frankfurter.dev` (ECB-derived, no API key), cached in a new `exchange_rates` table, with a safe administrator-entered manual fallback.
- ~~Whether "today's rate" should ever be used for *unconfirmed/in-progress* orders~~ **Resolved**: no. A rate is only ever resolved and snapshotted at the moment `createSaleFromOrder()` actually completes a sale — nothing before that point shows a USD-equivalent figure, and nothing needs to.
...
## 13. Phase 3 — EUR/USD currency design, as implemented (2026-08-17)

Implements the design evaluated in §11, exactly as recommended there (option (a): stored, frozen USD columns rather than computing on read), plus the confirmed final currency-scope rule that supersedes §4's original nuance:

**Confirmed rule (owner, 2026-08-17):** customers always see/pay EUR (Menu, Cart, Checkout, Orders — untouched). Inventory, recipe costs, packaging costs, Sales, profit, margin, and Analytics all report in USD. Since ingredient/recipe/packaging costs were already USD-denominated in this codebase (Inventory's existing `usd()` formatter), the only genuine *conversion* needed is EUR revenue → USD revenue on Sales/Analytics — cost columns require no change at all.

**Schema** (`supabase/migrations/20260817130847_...sql`):
- `exchange_rates(rate_date primary key, reference_date, rate, source, fetched_at)` — a cache of EUR→USD daily reference rates. `rate_date` is the date looked up *for*; `reference_date` is the actual ECB reference date used (earlier than `rate_date` on weekends/holidays, since no rate is published then). `source` is `'ecb_frankfurter'` or `'manual'`.
- `sales` gains `exchange_rate`, `exchange_rate_date`, `exchange_rate_source`, `usd_revenue`, `usd_profit` — all new columns. `sales.revenue` is never repurposed or converted in place; it stays the frozen, original EUR amount permanently.
- `sale_items` gains `usd_line_revenue`, `usd_line_profit` — same principle; `line_revenue` stays EUR, frozen.

**Rate source** (`js/currency-conversion.js`): `api.frankfurter.dev` — republishes the European Central Bank's own daily reference rates, requires no API key (so there is no secret to protect, in browser code or in Git), and is CORS-open (`access-control-allow-origin: *`, verified directly). Requesting a weekend/holiday date returns the latest valid business-day reference rate automatically (ECB's own convention — verified directly: requesting a Saturday returns the preceding Friday's rate), so no separate weekend/holiday logic is needed.

**Resolution order, every time a sale is completed** (`resolveExchangeRate()`): (1) an already-cached rate for today's date; (2) a live fetch from Frankfurter, cached on success; (3) a `prompt()`-based manual entry from the admin, saved with `source: 'manual'`. If all three fail (no cache, fetch fails, admin cancels the prompt), the sale is **not** created and the order's status is reverted to whatever it was before — never a fabricated or default rate, and never a half-completed sale.

**Same rate for the whole sale:** the rate is resolved once and passed to `applyRateToSaleLines()`, which converts every one of that sale's lines with it. Rounding each line independently can legitimately disagree by a cent with rounding the sale's own total once (confirmed against real data: 10 of the 34 historical sales hit this) — resolved by assigning any rounding residual to the line with the largest EUR revenue per sale, which by construction can never be a Mix & Match child line (always EUR revenue 0), so item-level and sale-level USD figures always reconcile exactly and a child line can never inherit a stray cent of "phantom" revenue.

**Historical backfill:** the 34 existing sales' distinct completion dates (7 dates) were looked up against Frankfurter and cached, then every sale's `usd_revenue`/`usd_profit`/`exchange_rate*` and every `sale_items` row's `usd_line_revenue`/`usd_line_profit` were computed from each sale's own already-frozen `revenue`/`line_revenue`/`total_cost` — never today's rate applied retroactively, and never touching `orders`, `order_items`, inventory, recipes, or customer data. Verified live: 34/34 sales backfilled, total `usd_revenue` = $980.55, total `usd_profit` = $731.16 (independently hand-verified beforehand and matched exactly), 0 reconciliation mismatches.

**Historical results never change when exchange rates later change:** by construction — `usd_revenue`/`usd_profit`/`usd_line_revenue`/`usd_line_profit` are stored, frozen values, computed once from the rate snapshotted at that sale's own completion time. A later addition to `exchange_rates` (a new date, or even a manual override of a past date) never touches an already-written sale.

Full test coverage in `tests/currency-conversion.test.js` (23 tests: rounding, EUR→USD conversion, sale- and item-level figures, the same-rate-for-the-whole-sale guarantee and its rounding-residual edge case, Mix & Match safety, weekend/holiday reference-rate resolution, API failure and manual fallback, zero revenue, and missing/invalid-rate handling).
